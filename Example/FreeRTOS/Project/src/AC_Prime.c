	...
	loop2 :
		...
		// {
			...
			// {
				...
				// vTaskDelay( 50/portTICK_RATE_MS ) ;
			// } 
			// temp.eding = Buf_A1_IN.btB & FETCH_EDING ;
			// temp.adress_allow_set = Buf_A1_IN.btD & FETCH_AAS ;
			// temp.jingyadangshu = Buf_A1_IN.btE & FETCH_JYDS ;
			// if( 0 != memcmp( &Registing[id].speset, &temp, sizeof(SPECIAL_SETTING) ) ) 
				// memcpy( &Registing[id].speset, &temp, sizeof(SPECIAL_SETTING) ) ;  
			// else
			// {
				// Set_a1twice( id, 0 ) ;
				// return 0 ;       
...
loop2:
	...
	// {	
		...
		// memcpy( &Registing[i].machineparam, ((u8*)&MSG_A2_IN) + 2 ), sizeof(MAC_PARAM) ) ;
		// Set_a2param( i, 0 ) ; 
		// return 0 ;     
	// }
